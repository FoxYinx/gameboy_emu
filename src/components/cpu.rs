    ...
    pub(crate) fn process_opcode(&mut self, opcode: u8, memory: &mut Memory) -> (bool, u64) {
        ...
        match opcode { 
            ...
            0x15 => {
                ...
                if self.debug_instructions {
                    println!("Opcode: {:#04X} DEC D, D now is {:#04X}, at PC {:#06X}", opcode, self.registers.d, self.registers.pc);
                }
            ...
            0xB0 => {
                if self.debug_instructions {
                    println!("Opcode: {:#04X} OR A B, A = {:#04X}, B = {:#04X}, at PC {:#06X}", opcode, self.registers.a, self.registers.b, self.registers.pc);
                }
            ...
            0xB2 => {
                if self.debug_instructions {
                    println!("Opcode: {:#04X} OR A D, A = {:#04X}, D = {:#04X}, at PC {:#06X}", opcode, self.registers.a, self.registers.d, self.registers.pc);
                }
            ...
            0xB3 => {
                if self.debug_instructions {
                    println!("Opcode: {:#04X} OR A E, A = {:#04X}, E = {:#04X}, at PC {:#06X}", opcode, self.registers.a, self.registers.e, self.registers.pc);
                }
            ...
            0xB4 => {
                if self.debug_instructions {
                    println!("Opcode: {:#04X} OR A H, A = {:#04X}, H = {:#04X}, at PC {:#06X}", opcode, self.registers.a, self.registers.h, self.registers.pc);
                }
            ...
            0xB5 => {
                if self.debug_instructions {
                    println!("Opcode: {:#04X} OR A L, A = {:#04X}, L = {:#04X}, at PC {:#06X}", opcode, self.registers.a, self.registers.l, self.registers.pc);
                }
                ...
                if self.registers.get_z() {
                    ...
                    if let Some(low) = memory.get(self.registers.pc as usize) {
                        ...
                        if let Some(high) = memory.get(self.registers.pc as usize) {
                            ...
                            if self.debug_instructions {
                                println!("Opcode: {:#04X} CALL Z a16, with a16 = {:#06X}, at PC {:#06X}", opcode, address, self.registers.pc.wrapping_sub(2));
                            }
                ...
                if !self.registers.get_c() {
                    ...
                    if let Some(low) = memory.get(self.registers.pc as usize) {
                        ...
                        if let Some(high) = memory.get(self.registers.pc as usize) {
                            ...
                            if self.debug_instructions {
                                println!("Opcode: {:#04X} JP NC a16, with a16 = {:#06X}, at PC {:#06X}", opcode, address, self.registers.pc.wrapping_sub(2));
                            }
                    ...
                    if self.debug_instructions {
                        println!("Opcode: {:#04X} JP NC a16 but C was true, at PC {:#06X}", opcode, self.registers.pc);
                    }
                ...
                if !self.registers.get_c() {
                    ...
                    if let Some(low) = memory.get(self.registers.pc as usize) {
                        ...
                        if let Some(high) = memory.get(self.registers.pc as usize) {
                            ...
                            if self.debug_instructions {
                                println!("Opcode: {:#04X} CALL NC a16, with a16 = {:#06X}, at PC {:#06X}", opcode, address, self.registers.pc.wrapping_sub(2));
                            }
            ...
            0xD9 => {
                if let Some(low) = memory.get(self.registers.sp as usize) {
                    ...
                    if let Some(high) = memory.get(self.registers.sp as usize) {
                        ...
                        if self.debug_instructions {
                            println!("Opcode: {:#04X} RETI to {:#06X}, PC was {:#06X}", opcode, return_address, self.registers.pc);
                        }
                ...
                if self.registers.get_c() {
                    ...
                    if let Some(low) = memory.get(self.registers.pc as usize) {
                        ...
                        if let Some(high) = memory.get(self.registers.pc as usize) {
                            ...
                            if self.debug_instructions {
                                println!("Opcode: {:#04X} CALL C a16, with a16 = {:#06X}, at PC {:#06X}", opcode, address, self.registers.pc.wrapping_sub(2));
                            }
